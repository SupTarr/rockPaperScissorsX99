    ...
    .fadeOut(VIEW_TRANSITION_TIME, () => {
      ...
      if (roundNumber < 3) {
        ...
        $("#winner")
          .css("display", "flex")
          .hide()
          .fadeIn(VIEW_TRANSITION_TIME, () => $WIN_SOUND.play());
      }
    });
}
...
function updateMoveValue(moveInput, moveInputId) {
  ...
  const unchangedValueSum =
    Number($("#" + unchangedValueIds[0]).val()) +
...
function getWinnerText(winner) {
  if (winner === PLAYER_ONE) {
    ...
    return playerMode === COMPUTER_PLAYER_MODE
      ? "Computer Player Won!"
      : "Player Two Won!";
  } else if (winner === TIE) {
    return "Tie!";
